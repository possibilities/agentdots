...
test_current="$test_root/current-theme"
...
case "${1:-}:${2:-}" in
    ...
    notification:show)
        [ "${4:-}" = '--sound' ] && [ "${5:-}" = none ] \
            || { printf 'unexpected notification arguments: %s\n' "$*" >&2; exit 2; }
        printf 'notify %s\n' "${3:-}" >>"${TEST_HERDR_LOG:?}"
        [ "${TEST_HERDR_RELOAD_FAIL:-0}" != 1 ] || exit 1
        ;;
    *)
...
case "${1:-}" in
    list)
        printf '%s\n' base16-one base24-two tinted8-three
        ;;
    current)
        [ -s "${TEST_TINTY_CURRENT:?}" ] || exit 1
        cat "$TEST_TINTY_CURRENT"
        ;;
    apply)
        [ "$#" -eq 2 ] || exit 2
        printf '%s\n' "$2" >"${TEST_TINTY_CURRENT:?}"
        printf 'apply %s lock=%s\n' "$2" "${AGENTSTART_HERDR_TINTY_LOCK_HELD:-0}" \
            >>"${TEST_TINTY_LOG:?}"
    ...
    build)
        ...
        cp "${TEST_THEME_FRAGMENT:?}" "$2/themes/base24-fixture.toml"
        cp "${TEST_THEME_FRAGMENT:?}" "$2/themes/tinted8-fixture.toml"
        printf 'build %s\n' "$2" >>"${TEST_TINTY_LOG:?}"
        ;;
    init)
        if [ ! -s "${TEST_TINTY_CURRENT:?}" ]; then
            printf 'base16-one\n' >"$TEST_TINTY_CURRENT"
        fi
        "$0" apply "$(cat "$TEST_TINTY_CURRENT")"
        ;;
    *)
...
export TEST_TINTY_CURRENT="$test_current"
...
# migrates the exact Funk-owned Herdr symlink when Tinty initializes.
...
[ -f "$AGENTSTART_TINTY_TEMPLATE_TARGET/themes/tinted8-fixture.toml" ] \
    || fail "install did not activate built Tinted8 themes"
...
grep -Fqx 'notify Theme: Fixture One' "$test_log" \
    || fail "initial theme application did not notify with its display name"
...
# Reapplying the same palette stays quiet; changing it sends one silent Herdr
# notification using the generated scheme's human-readable name.
: >"$test_log"
"$helper" apply-theme "$theme_fragment" >/dev/null
if grep -F 'notify ' "$test_log" >/dev/null; then
    fail "reapplying the current theme sent a duplicate notification"
fi
"$helper" apply-theme "$alternate_fragment" >/dev/null
grep -Fqx 'notify Theme: Fixture Two' "$test_log" \
    || fail "theme change did not notify with its display name"
...
# Both directions wrap around the complete list. The nested Tinty hook inherits
# the held-lock marker, proving that cycling does not deadlock itself.
printf 'tinted8-three\n' >"$test_current"
"$helper" next >/dev/null
[ "$(cat "$test_current")" = 'base16-one' ] \
    || fail "next did not wrap from the final Tinty theme"
"$helper" previous >/dev/null
[ "$(cat "$test_current")" = 'tinted8-three' ] \
    || fail "previous did not wrap from the first Tinty theme"
[ "$(grep -c 'lock=1' "$test_tinty_log")" -ge 2 ] \
    || fail "Tinty cycle hooks did not inherit the held-lock marker"
